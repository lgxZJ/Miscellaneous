...
bool WavPlayer::supportsEffect3D() const
{
    assert (fileSet());
...
void WavPlayer::enableEffect3D(bool ifEnable)
{
    ...
    if (m_3dInterface->SetMode(mode, DS3D_IMMEDIATE) != DS_OK)
        throw std::exception("SetMode error");
}

//////////////////////////////////////////////////////////////////////////////////

Effect3DController WavPlayer::get3DController()
{
    assert(m_3dInterface != nullptr);
    return Effect3DController(m_3dInterface);
}
...
void WavPlayer::createBufferOfSeconds(unsigned seconds)
{
    ...
    if (m_soundBufferInterface->QueryInterface(IID_IDirectSound3DBuffer, &m_3dInterface) != DS_OK) {
