...
public:
    ...
    struct ConeAngles {
        int32_t insideAngle;
        int32_t outsideAngle;
    };

    Effect3DController(IDirectSound3DBuffer* interfacePtr)
    ...
    void        setSourcePos(Vector3D pos);

    ConeAngles  getConeAngles() const;
    void        setConeAngles(ConeAngles angles);
    Vector3D    getConeOrientation() const;
    void        setConeOrientation(Vector3D vector3D);
    long        getOutsideVolume() const;
...
private:
    IDirectSound3DBuffer*   m_interface;
};
...
public:
    ...
    bool supportsEffect3D() const;
    void enableEffect3D(bool ifEnable);
    Effect3DController get3DController();
...
private:
    ...
    IDirectSound3DBuffer8*  m_3dInterface;
